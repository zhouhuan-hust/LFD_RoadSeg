...
class dataStructure: 
    ...
    '''
    # cats = ['um_road', 'umm_road', 'uu_road', 'URBAN']
    cats = ['um', 'umm', 'uu', 'URBAN']
...
#########################################################################
def main(result_dir, 
train_dir, debug = False, eval_all=False):
    '''
    ...
    print_table = {key:[] for key in dataStructure.eval_propertyList}
    print_table['Benchmark'] = dataStructure.cats
    
    for cat in dataStructure.cats:
        # print( "Execute evaluation for category %s ..." %cat)
        fn_search  = '%s*%s' %(cat, dataStructure.gt_end)
        if cat == 'URBAN':
            fn_search = '*%s' % (dataStructure.gt_end)
        ...
        for fn_curGt in gt_fileList:
            ...
            # fn_curProb = os.path.join(submission_dir, dataset_tag + '_' + ts_tag + dataStructure.prob_end)
        ...
        if category_ok:
            ...
            for property in dataStructure.eval_propertyList:
                print_table[property].append('%4.2f' %(prob_eval_scores[-1][property]*factor))
    ...
    if not eval_all:
        ...
        print(pd.DataFrame.from_dict(print_table).loc[:, ['Benchmark', 'MaxF', 'AvgPrec', 'PRE_wp', 'REC_wp', 'FPR_wp', 'FNR_wp' ] ])
