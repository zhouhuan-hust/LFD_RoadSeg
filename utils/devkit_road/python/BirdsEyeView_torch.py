...
class BirdsEyeView(object):
    ...
    def transformImage2BEV(self, inImage, out_dtype = 'f4'):
        ...
        :param inImage:
        '''
        assert self.im_u_float.any(None)
        assert self.im_v_float.any(None)
        assert self.bev_x_ind.any(None)
        assert self.bev_z_ind.any(None)
        
        
        if len(inImage.shape) > 2:
            outputData = torch.zeros(self.bevParams.bev_size + (inImage.shape[2],), dtype = out_dtype).to(inImage.device)
            for channel in range(0, inImage.shape[2]):
                outputData[self.bev_z_ind-1, self.bev_x_ind-1, channel] = inImage[torch.from_numpy(self.im_v_float.astype('long')-1), 
                                                                                        torch.from_numpy(self.im_u_float.astype('long')-1), channel]
        else:
            outputData = torch.zeros(self.bevParams.bev_size, dtype = out_dtype).to(inImage.device)
            outputData[self.bev_z_ind-1, self.bev_x_ind-1] = inImage[torch.from_numpy(self.im_v_float.astype('long')-1), 
                                                                                        torch.from_numpy(self.im_u_float.astype('long')-1)]
